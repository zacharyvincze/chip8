...
bool Configurations::hasConfiguration(ConfigurationType configurationType) {
    ...
    LevelHelper::forEachLevel(&lIndex, [&](void) -> bool {
        if (hasConfiguration(LevelHelper::castFromInt(lIndex),
                             configurationType)) {
            result = true;
        }
        return result;
...
void Configurations::Parser::ignoreComments(std::string* line) {
    ...
    if (quotesStart != std::string::npos) {
        ...
        while (quotesEnd != std::string::npos &&
               line->at(quotesEnd - 1) == '\\') {
            // Do not erase slash yet - we will erase it in parseLine(..) while
            // loop
    ...
    if ((foundAt = line->find(base::consts::kConfigurationComment)) !=
        ...
        if (foundAt < quotesEnd) {
            foundAt =
                line->find(base::consts::kConfigurationComment, quotesEnd + 1);
        }
...
void Logger::configure(const Configurations& configurations) {
    ...
    if (m_typedConfigurations != nullptr) {
        Configurations* c = const_cast<Configurations*>(
...
void Logger::resolveLoggerFormatSpec(void) const {
    ...
    LevelHelper::forEachLevel(&lIndex, [&](void) -> bool {
        base::LogFormat* logFormat =
            const_cast<base::LogFormat*>(&m_typedConfigurations->logFormat(
                LevelHelper::castFromInt(lIndex)));
...
void File::buildStrippedFilename(const char* filename, char buff[],
    ...
    if (sizeOfFilename >= limit) {
        ...
        if (filename[0] != '.' &&
            filename[1] != '.') {  // prepend if not already
            filename += 3;         // 3 = '..'
            STRCAT(buff, "..", limit);
        }
    }
...
bool OS::termSupportsColor(void) {
    ...
    return term == "xterm" || term == "xterm-color" ||
           ...
           term == "cygwin" || term == "screen-256color";
}
...
base::type::string_t DateTime::formatTime(unsigned long long time,
    ...
    for (base::type::EnumType i = start;
        ...
        time /=
            static_cast<decltype(time)>(base::consts::kTimeFormats[i].value);
        ...
        if (*format == base::consts::kFormatSpecifierChar) {
            switch (*++format) {
                ...
                case '\0':  // End
                    ...
                    break;
                case 'd':  // Day
                    buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_mday,
                                                                2, buf, bufLim);
                    continue;
                ...
                case 'h':  // hour (12-hour)
                    ...
                    continue;
                case 'H':  // hour (24-hour)
                    buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_hour,
                                                                2, buf, bufLim);
                    continue;
                case 'm':  // minute
                    buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_min,
                                                                2, buf, bufLim);
                    continue;
                case 's':  // second
                    buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_sec,
                                                                2, buf, bufLim);
                    continue;
                case 'z':  // subsecond part
                case 'g':
                    buf = base::utils::Str::convertAndAddToBuff(
                        msec, ssPrec->m_width, buf, bufLim);
                    continue;
                case 'F':  // AM/PM
                    buf = base::utils::Str::addToBuff((tInfo->tm_hour >= 12)
                                                          ? base::consts::kPm
                                                          : base::consts::kAm,
                                                      buf, bufLim);
                    continue;
...
void LogFormat::parseFromFormat(const base::type::string_t& userFormat) {
    ...
    auto conditionalAddFlag = [&](const base::type::char_t* specifier,
        ...
        while ((foundAt = formatCopy.find(specifier, foundAt + 1)) !=
            ...
            if (foundAt > 0 &&
                ...
                if (hasFlag(flag)) {
                    // If we already have flag we remove the escape chars so
                    // that '%%' is turned to '%' even after specifier
                    // resolution - this is because we only replaceFirst
                    // specifier
    ...
    if ((dateIndex = formatCopy.find(base::consts::kDateTimeFormatSpecifier)) !=
        std::string::npos) {
        while (dateIndex > 0 && formatCopy[dateIndex - 1] ==
...
void LogFormat::updateDateFormat(std::size_t index,
    ...
    if ((currFormat.size() > index) && (ptr[0] == '{')) {
        ...
        if (hasFlag(base::FormatFlags::DateTime)) {
            m_dateTimeFormat =
                std::string(base::consts::kDefaultDateTimeFormat);
        }
    }
}
...
void TypedConfigurations::build(Configurations* configurations) {
    ...
    for (Configurations::const_iterator it = configurations->begin();
        ...
        // We cannot use switch on strong enums because Intel C++ dont support
        // them yet
        if (conf->configurationType() == ConfigurationType::Enabled) {
            ...
            // loop. This is because if file cannot be created, we will force
            // ToFile to be false. Because configuring logger is not necessarily
            // performance sensative operation, we can live with another loop;
            // (by the way this loop is not very heavy either)
...
unsigned long TypedConfigurations::getULong(std::string confVal) {
    ...
    if (!valid) {
        valid = false;
        ELPP_ASSERT(valid, "Configuration value not a valid integer ["
                               << confVal << "]");
        return 0;
    }
    ...
    if ((dateIndex = resultingFilename.find(
        ...
        if (dateIndex != std::string::npos) {
            ...
            if ((resultingFilename.size() > dateIndex) && (ptr[0] == '{')) {
                ...
                fmt =
                    std::string(base::consts::kDefaultDateTimeFormatInFilename);
            }
            base::SubsecondPrecision ssPrec(3);
            std::string now =
                base::utils::DateTime::getDateTime(fmt.c_str(), &ssPrec);
            base::utils::Str::replaceAll(now, '/',
                                         '-');  // Replace path element since we
                                                // are dealing with filename
...
void TypedConfigurations::insertFile(Level level,
    ...
    if (resolvedFilename.empty()) {
        std::cerr << "Could not load empty file for logging, please re-check "
                     "your configurations for level ["
    ...
    auto create = [&](Level level) {
        ...
        if (filestreamIter == m_logStreamsReference->end()) {
            ...
            m_fileStreamMap.insert(
                std::make_pair(level, base::FileStreamPtr(fs)));
            m_logStreamsReference->insert(
                std::make_pair(resolvedFilename,
            ...
            m_fileStreamMap.insert(std::make_pair(
                level, base::FileStreamPtr(filestreamIter->second)));
    ...
    // If we dont have file conf for any level, create it for Level::Global
    // first otherwise create for specified level
...
bool TypedConfigurations::unsafeValidateFileRolling(
    ...
    if (maxLogFileSize != 0 && currFileSize >= maxLogFileSize) {
        std::string fname =
            unsafeGetConfigByRef(level, &m_filenameMap, "filename");
        ELPP_INTERNAL_INFO(
            1, "Truncating log file ["
...
bool RegisteredHitCounters::validateAfterN(const char* filename,
    ...
    // here Note the >= instead of > because we are incrementing after this
    // check
...
Logger* RegisteredLoggers::get(const std::string& id, bool forceCreation) {
    ...
    if (logger_ == nullptr && forceCreation) {
        ...
        logger_ =
            new Logger(id, m_defaultConfigurations, &m_logStreamsReference);
        ...
        switch (*modules) {
            ...
            default:
                if (isMod) {
                    ...
                    if (isdigit(*modules)) {
                        level =
                            static_cast<base::type::VerboseLevel>(*modules) -
                            48;
                    }
                }
                break;
...
bool VRegistry::allowed(base::type::VerboseLevel vlevel, const char* file) {
    ...
    if (m_modules.empty() || file == nullptr) {
        ...
        for (; it != m_modules.end(); ++it) {
            if (base::utils::Str::wildCardMatch(baseFilename,
                                                it->first.c_str())) {
...
void Storage::setApplicationArguments(int argc, char** argv) {
    ...
    if (m_commandLineArgs.hasParamWithValue(base::consts::kLoggingFlagsParam)) {
        int userInput = atoi(
            m_commandLineArgs.getParamValue(base::consts::kLoggingFlagsParam));
...
void LogDispatchCallback::handle(const LogDispatchData* data) {
    ...
    if (lock == m_fileLocks.end()) {
        m_fileLocks.emplace(
            std::make_pair(filename, std::unique_ptr<base::threading::Mutex>(
...
void DefaultLogDispatchCallback::dispatch(base::type::string_t&& logLine) {
    if (m_data->dispatchAction() == base::DispatchAction::NormalLog) {
        if (m_data->logMessage()->logger()->m_typedConfigurations->toFile(
            ...
            base::type::fstream_t* fs = m_data->logMessage()
                                            ->logger()
                                            ->m_typedConfigurations->fileStream(
                                                m_data->logMessage()->level());
            if (fs != nullptr) {
                ...
                if (fs->fail()) {
                        ...
                        "Unable to write log to file ["
                            << m_data->logMessage()
                                   ...
                                   ->m_typedConfigurations->filename(
                                       m_data->logMessage()->level())
                            << "].\n"
                            << "Few possible reasons (could be something "
                               "else):\n"
                            << "      * Permission denied\n"
                            << "      * Disk full\n"
                    ...
                    if (ELPP->hasFlag(LoggingFlag::ImmediateFlush) ||
                        (m_data->logMessage()->logger()->isFlushNeeded(
                            m_data->logMessage()->level()))) {
                        m_data->logMessage()->logger()->flush(
                            m_data->logMessage()->level(), fs);
                    }
                }
            } else {
                ELPP_INTERNAL_ERROR(
                    "Log file for [" << LevelHelper::convertToString(
                                            m_data->logMessage()->level())
                                     << "] "
                                     << "has not been configured but [TO_FILE] "
                                        "is configured to TRUE. [Logger ID: "
                                     << m_data->logMessage()->logger()->id()
                                     << "]",
                    false);
            }
        }
        if (m_data->logMessage()
                ->logger()
                ->m_typedConfigurations->toStandardOutput(
                    m_data->logMessage()->level())) {
            if (ELPP->hasFlag(LoggingFlag::ColoredTerminalOutput))
                m_data->logMessage()
                    ->logger()
                    ->logBuilder()
                    ->convertToColoredOutput(&logLine,
                                             m_data->logMessage()->level());
            ...
            if (fs != nullptr) {
                ...
                ELPP_INTERNAL_ERROR(
                    "Log file for ["
                        ...
                        << "] "
                        << "has not been configured but [TO_FILE] is "
                           "configured to TRUE. [Logger ID: "
                        << logger->id() << "]",
                    false);
            }
        }
...
base::type::string_t DefaultLogBuilder::build(const LogMessage* logMessage,
    ...
    if (logFormat->hasFlag(base::FormatFlags::Function)) {
        // Function
        base::utils::Str::replaceFirstWithEscape(
            logLine, base::consts::kLogFunctionFormatSpecifier,
            logMessage->func());
    }
    if (logFormat->hasFlag(base::FormatFlags::File)) {
        ...
        base::utils::File::buildStrippedFilename(logMessage->file().c_str(),
                                                 buff);
        base::utils::Str::replaceFirstWithEscape(
    ...
    if (logFormat->hasFlag(base::FormatFlags::FileBase)) {
        ...
        base::utils::Str::replaceFirstWithEscape(
            logLine, base::consts::kLogFileBaseFormatSpecifier,
            std::string(buff));
    }
    ...
    if (logFormat->hasFlag(base::FormatFlags::Location)) {
        ...
        buf = base::utils::Str::convertAndAddToBuff(
            ...
            false);
        base::utils::Str::replaceFirstWithEscape(
            logLine, base::consts::kLogLocationFormatSpecifier,
            std::string(buff));
    }
    if (logMessage->level() == Level::Verbose &&
        ...
        buf = base::utils::Str::convertAndAddToBuff(logMessage->verboseLevel(),
                                                    1, buf, bufLim, false);
        base::utils::Str::replaceFirstWithEscape(
            logLine, base::consts::kVerboseLevelFormatSpecifier,
            std::string(buff));
    }
    if (logFormat->hasFlag(base::FormatFlags::LogMessage)) {
        // Log message
        base::utils::Str::replaceFirstWithEscape(
            logLine, base::consts::kMessageFormatSpecifier,
            logMessage->message());
    }
...
void LogDispatcher::dispatch(void) {
    ...
    if (ELPP->hasFlag(LoggingFlag::StrictLogFileSizeCheck)) {
        tc->validateFileRolling(m_logMessage->level(),
    ...
    if (m_logger == nullptr) {
        {
            if (!ELPP->registeredLoggers()->has(
                ...
                // Somehow default logger has been unregistered. Not good!
                // Register again
    ...
    if (m_enabled) {
        ...
        if (m_scopedLog) {
            ...
            PerformanceTrackingData data(
                PerformanceTrackingData::DataType::Complete);
            data.init(this);
            data.m_formattedTimeTaken = formattedTime;
            PerformanceTrackingCallback* callback = nullptr;
            for (const std::pair<std::string,
                                 base::type::PerformanceTrackingCallbackPtr>&
    ...
    if (m_enabled) {
        ...
        base::type::string_t formattedTime =
            m_hasChecked ? getFormattedTimeTaken(m_lastCheckpointTime)
                         : ELPP_LITERAL("");
        PerformanceTrackingData data(
            PerformanceTrackingData::DataType::Checkpoint);
        data.init(this);
    ...
    if (size > kStackStart) {  // Skip StackTrace c'tor and generateNew
        for (std::size_t i = kStackStart; i < size; ++i) {
            ...
            if (!mangName.empty()) {
                int status = 0;
                char* demangName =
                    abi::__cxa_demangle(mangName.data(), 0, 0, &status);
                ...
                if (status == 0) {
                    ...
                    StackTraceEntry entry(i - 1, location, demangName, hex,
                                          addr);
    ...
    for (int i = 0; i < base::consts::kCrashSignalsCount; ++i) {
        if (base::consts::kCrashSignals[i].numb == sig) {
            ...
            if (ELPP->hasFlag(el::LoggingFlag::LogDetailedCrashReason)) {
                ss << std::endl
                   << "    " << base::consts::kCrashSignals[i].brief
                   << std::endl
...
void Loggers::configureFromGlobal(const char* globalConfigurationFilePath) {
    ...
    ELPP_ASSERT(gcfStream.is_open(),
                "Unable to open global configuration file ["
